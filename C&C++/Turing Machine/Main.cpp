...
void checkFirst() {
  ...
  cout << t1.getCurrentState() << " " << t1.getCurrentContent() << " "
       << t1.getNextState() << " " << t1.getNextContent() << " "
       << t1.getMoveDirection() << endl;
  cout << t2.getCurrentState() << " " << t2.getCurrentContent() << " "
       << t2.getNextState() << " " << t2.getNextContent() << " "
       << t2.getMoveDirection() << endl;
}
...
bool compareState(TuringMachineState s1, TuringMachineState s2) {
  if (s1.getCurrentState() == s2.getCurrentState()) {
    if (s1.getCurrentContent() < s2.getCurrentContent()) {
      return (s1.getCurrentState() < s2.getCurrentState()) || true;
    } else {
