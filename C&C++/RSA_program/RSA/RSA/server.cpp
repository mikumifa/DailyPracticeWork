...
int main(int argc, char* argv[]) {
    ...
    int q = 29;
    int n_ca, e_ca, d_ca;
    generateKeys(p, q, n_ca, e_ca, d_ca);
    std::cout << "Public key: (" << e_ca << ", " << n_ca << ")" << std::endl;
    std::cout << "Private key: (" << d_ca << ", " << n_ca << ")" << std::endl;
    //********************************************************************
...
//********************************************************************
        ...
        //TODO Sending encrypted RSA public key to the client
        int encrypted_e = encrypt(e_ca, d_ca, n_ca);
        int encrypted_n = encrypt(n_ca, d_ca, n_ca);
        sprintf(send_buffer, "RSA_PUBLIC_KEY:%d:%d\r\n", encrypted_e, encrypted_n);
