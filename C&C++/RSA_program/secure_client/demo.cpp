...
// remember that the BUFFESIZE has to be at least big enough to receive the
// answer from the server
#define SEGMENT_SIZE 70
  // segment size, i.e., if fgets gets more than this number of bytes it
  ...
  */
  ...
  if (argc == 3) {
    ...
    // iResult = getaddrinfo("0:0:0:0:0:0:0:1", portNum, &hints, &result);
    // //works! test only!
  } else {
  ...
  if (connect(s, result->ai_addr, result->ai_addrlen) != 0) {
    ...
    //~ serverHost, sizeof(serverHost),
    //~ serverService, sizeof(serverService),
    //~ NI_NAMEREQD); //works only if the DNS can resolve the hostname;
  ...
  int encrypted_e, encrypted_n;
  sscanf(receive_buffer, "RSA_PUBLIC_KEY:%d:%d", &encrypted_e, &encrypted_n);

  // 解密RSA公钥
  int public_e = decrypt(encrypted_e, d_ca, n_ca);
  int public_n = decrypt(encrypted_n, d_ca, n_ca);

  printf("RSA Public Key received: (%d, %d)\n", public_e, public_n);
